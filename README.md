...
- 读取不删除服务端数据；明细持久化到浏览器 IndexedDB（最多保留 10000 条），重开页面自动恢复
- Management key 记入 localStorage（XOR + Base64 混淆存储），下次打开页面自动填入
- 作为静态页面运行，不需要额外后端服务
...
- 仓库中不包含 Management key。使用时请在浏览器中输入你自己的密钥；密钥会以混淆形式保存在本浏览器的 localStorage 中，下次自动填入。
- 统计汇总读取自插件的服务端数据库，读取不会删除数据，可随时重新刷新。
- 请求明细接口每次只返回最近若干条，页面会累积并持久化到浏览器 IndexedDB（最多 10000 条），重开页面自动恢复；“清空页面”会同时删除本地明细数据库。
